...
- **Leaked tool-call syntax filter** — llama-3.3 sometimes writes its native tool-call format (`<function=name>{...}</function>`) into plain answer text instead of emitting structured `tool_calls` deltas, especially in the second call after a tool result. All streamed content passes through a stateful filter (`createFunctionTagFilter` in `groq.js`) that strips these tags, including ones split across stream deltas, so the syntax never reaches the widget.
